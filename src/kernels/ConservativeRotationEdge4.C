...
#include "ConservativeRotationEdge4.h"
#include "SystemBase.h"
#include "libmesh/utility.h"

registerMooseObject("TransEQApp", ConservativeRotationEdge4);

InputParameters
ConservativeRotationEdge4::validParams()
{
  ...
  params.addCoupledVar("rho_screw_4", "The variable representing the quandrant 1 edge dislocation density.");
  return params;
}

ConservativeRotationEdge4::ConservativeRotationEdge4(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _upwind_node(0),
    _dtotal_mass_out(0),
    _rho_screw_4(coupledValue("rho_screw_4"))
{
}

Real
ConservativeRotationEdge4::negSpeedQp()
{
  Real edge_sign;
  Real screw_sign;
  Real vel;
  ...
  for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
  {
    return _rho_screw_4[_qp] * (- _grad_test[_i][_qp] * _scale * RealVectorValue(_velocity[j], 0.0, 0.0) + _grad_test[_i][_qp] * RealVectorValue(0.0, _velocity[j], 0.0));
  }
...
Real
ConservativeRotationEdge4::computeQpResidual()
{
...
Real
ConservativeRotationEdge4::computeQpJacobian()
{
  // This is the no-upwinded version
...
void
ConservativeRotationEdge4::computeResidual()
{
  switch (_upwinding)
  {
...
void
ConservativeRotationEdge4::computeJacobian()
{
  switch (_upwinding)
  {
    case UpwindingType::none:
...
void
ConservativeRotationEdge4::fullUpwind(JacRes res_or_jac)
{
