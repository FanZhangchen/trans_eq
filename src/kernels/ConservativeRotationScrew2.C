...
#include "ConservativeRotationScrew2.h"
#include "SystemBase.h"
#include "libmesh/utility.h"

registerMooseObject("TransEQApp", ConservativeRotationScrew2);

InputParameters
ConservativeRotationScrew2::validParams()
{
  ...
  params.addCoupledVar("rho_edge_2", "The variable representing the quandrant 1 edge dislocation density.");
  return params;
}

ConservativeRotationScrew2::ConservativeRotationScrew2(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _upwind_node(0),
    _dtotal_mass_out(0),
    _rho_edge_2(coupledValue("rho_edge_2"))
{
}

Real
ConservativeRotationScrew2::negSpeedQp()
{
  Real edge_sign;
  Real screw_sign;
  ...
  for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
  {
    return _rho_edge_2[_qp] * (- _grad_test[_i][_qp] * _scale * RealVectorValue(_velocity[j], 0.0, 0.0) + _grad_test[_i][_qp] * RealVectorValue(0.0, _velocity[j], 0.0));
  }
  
}

Real
ConservativeRotationScrew2::computeQpResidual()
{
...
Real
ConservativeRotationScrew2::computeQpJacobian()
{
  // This is the no-upwinded version
...
void
ConservativeRotationScrew2::computeResidual()
{
  switch (_upwinding)
  {
...
void
ConservativeRotationScrew2::computeJacobian()
{
  switch (_upwinding)
  {
    case UpwindingType::none:
...
void
ConservativeRotationScrew2::fullUpwind(JacRes res_or_jac)
{
