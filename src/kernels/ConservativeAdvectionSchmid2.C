...
registerMooseObject("TransEQApp", ConservativeAdvectionSchmid2);

InputParameters
ConservativeAdvectionSchmid2::validParams()
{
...
Real
ConservativeAdvectionSchmid2::negSpeedQp()
{
  Real edge_sign;
  ...
  //   {
  //     _velocity[j] = _edge_slip_direction[_qp][_slip_sys_index * LIBMESH_DIM + j]; // edge
  //     direction
  //   }
  //   break;
  ...
  //   for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
  //   {
  //     _velocity[j] = _screw_slip_direction[_qp][_slip_sys_index * LIBMESH_DIM + j]; // screw
  //     direction
  //   }
  //   break;
  // }
  ...
  for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
  {
    _velocity[j] *= _dislo_velocity[_qp]; // velocity value
    _velocity[j] *= edge_sign;            // positive or negative dislocation
  }

  return -_grad_test[_i][_qp] * RealVectorValue(0.0, 0.0, 0.0);
}

Real
ConservativeAdvectionSchmid2::computeQpResidual()
{
...
Real
ConservativeAdvectionSchmid2::computeQpJacobian()
{
  // This is the no-upwinded version
...
void
ConservativeAdvectionSchmid2::computeResidual()
{
  switch (_upwinding)
  {
...
void
ConservativeAdvectionSchmid2::computeJacobian()
{
  switch (_upwinding)
  {
    case UpwindingType::none:
...
void
ConservativeAdvectionSchmid2::fullUpwind(JacRes res_or_jac)
{
