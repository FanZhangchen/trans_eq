...
#include "ConservativeRotationScrew3.h"
#include "SystemBase.h"
#include "libmesh/utility.h"

registerMooseObject("TransEQApp", ConservativeRotationScrew3);

InputParameters
ConservativeRotationScrew3::validParams()
{
  ...
  params.addCoupledVar("rho_edge_3", "The variable representing the quandrant 1 edge dislocation density.");
  return params;
}

ConservativeRotationScrew3::ConservativeRotationScrew3(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _upwind_node(0),
    _dtotal_mass_out(0),
    _rho_edge_3(coupledValue("rho_edge_3"))
{
}

Real
ConservativeRotationScrew3::negSpeedQp()
{
  Real edge_sign;
  Real screw_sign;
  ...
  for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
  {
    return _rho_edge_3[_qp] * (_grad_test[_i][_qp] * _scale * RealVectorValue(_velocity[j], 0.0, 0.0) + _grad_test[_i][_qp] * RealVectorValue(0.0, _velocity[j], 0.0));
  }

}

Real
ConservativeRotationScrew3::computeQpResidual()
{
...
Real
ConservativeRotationScrew3::computeQpJacobian()
{
  // This is the no-upwinded version
...
void
ConservativeRotationScrew3::computeResidual()
{
  switch (_upwinding)
  {
...
void
ConservativeRotationScrew3::computeJacobian()
{
  switch (_upwinding)
  {
    case UpwindingType::none:
...
void
ConservativeRotationScrew3::fullUpwind(JacRes res_or_jac)
{
