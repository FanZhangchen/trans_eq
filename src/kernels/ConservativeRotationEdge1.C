...
#include "ConservativeRotationEdge1.h"
#include "SystemBase.h"
#include "libmesh/utility.h"

registerMooseObject("TransEQApp", ConservativeRotationEdge1);

InputParameters
ConservativeRotationEdge1::validParams()
{
  ...
  params.addCoupledVar("rho_screw_1", "The variable representing the quandrant 1 edge dislocation density.");
  return params;
}

ConservativeRotationEdge1::ConservativeRotationEdge1(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _upwind_node(0),
    _dtotal_mass_out(0),
    _rho_screw_1(coupledValue("rho_screw_1"))
{
}

Real
ConservativeRotationEdge1::negSpeedQp()
{
  Real edge_sign;
  Real screw_sign;
  Real vel;
  ...
  for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
  {
    return _rho_screw_1[_qp] * (_grad_test[_i][_qp] * _scale * RealVectorValue(_velocity[j], 0.0, 0.0) + _grad_test[_i][_qp] * RealVectorValue(0.0, _velocity[j], 0.0));
  }
...
Real
ConservativeRotationEdge1::computeQpResidual()
{
...
Real
ConservativeRotationEdge1::computeQpJacobian()
{
  // This is the no-upwinded version
...
void
ConservativeRotationEdge1::computeResidual()
{
  switch (_upwinding)
  {
...
void
ConservativeRotationEdge1::computeJacobian()
{
  switch (_upwinding)
  {
    case UpwindingType::none:
...
void
ConservativeRotationEdge1::fullUpwind(JacRes res_or_jac)
{
