...
#include "ShearTraction.h"

#include "Assembly.h"
#include "Function.h"
#include "MooseError.h"

registerMooseObject("TensorMechanicsApp", ShearTraction);

InputParameters
ShearTraction::validParams()
{
...
ShearTraction::ShearTraction(const InputParameters & parameters)
...
ShearTraction::initialSetup()
{
  ...
  for (auto blk_id : block_ids)
  {
    if (_coord_type != _fe_problem.getCoordSystem(blk_id))
      mooseError("The ShearTraction BC requires subdomains to have the same coordinate system.");
  }
}
...
Real
ShearTraction::computeQpJacobian()
{
...
Real
ShearTraction::computeQpOffDiagJacobian(const unsigned int jvar_num)
{
  if (_use_displaced_mesh)
...
Real
ShearTraction::computeStiffness(const unsigned int coupled_component)
{
  if (_ndisp > 1)
  {
  ...
  {
    return computeFactor() * _tangents[_qp][0](_component) * _test[_i][_qp] * _phi[_j][_qp] *
           (2 / _q_point[_qp](0));
  }
...
ShearTraction::computeFaceStiffness(const unsigned int local_j, const unsigned int coupled_component)
{
  ...
  {
    rz_term = _tangents[_qp][0](i) * _phi[_j][_qp] / _q_point[_qp](0);
  }
...
Real
ShearTraction::computeFactor() const
{
  Real factor = _factor;
...
void
ShearTraction::precalculateQpJacobian()
{
  if (_ndisp == 1)
    return;
...
void
ShearTraction::precalculateQpOffDiagJacobian(const MooseVariableFEBase & /*jvar*/)
{
